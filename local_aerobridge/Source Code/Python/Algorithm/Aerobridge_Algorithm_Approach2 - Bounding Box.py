...
def run_exe(image_file_path):
    ...
    for exe in feature_list:
        ...
        print "EXE Output: " + result_str
        df , height_width = parse_data(result_str)
        print "Parsed Co-Ordinates:"
        print df
        print "\n****************************Finished :"+exe+".exe*********************************************************\n"
...
def parse_data(data):
    ...
    if len(data_list) > 1:
        ...
        print "Height,Width:"+ str(pixel_list) +"\n"   
...
def read_points_from_datalog(image_path, flightName,imageName):
    ...
    for feature in feature_list:
        ...
        print feature + flightName + imageName
        print df
...
def calculate_doorframe_parameters(subfeature_dict):
    ...
    if len(dfl_points_dict) == 3:
        ...
        #Find Origin of DoorFrame
        dfl_origin_X = min([dfl_points_dict['doorframelines1'][0] , dfl_points_dict['doorframelines3'][0]])
        dfl_origin_Y = min([dfl_points_dict['doorframelines1'][1] , dfl_points_dict['doorframelines3'][1]])
        dfl_origin= [dfl_origin_X,dfl_origin_Y]
...
def plot_points(image_path, subfeature_dict, subfeature_valid_list  ,meanpoint = [],Title = '' ):
    ...
    if (plot_enable or 'Raw Image:' in Title):
        ...
        ax = plt.subplots(1)
        plt.imshow(im)
        right_bot_x, right_bot_y, left_top_x, left_top_y, h, w = 0, 0, 100000, 1000000, 0 ,0
        ...
        for item in subfeature_dict:
            ...
            if not (x == []):
                ...
                plt.annotate(item, xy=(x, y), xytext=(-20, 20),size=20, textcoords='offset points', ha='right', va='bottom', bbox=dict(boxstyle='round,pad=0.25', fc='white', alpha=0.35), arrowprops=dict(arrowstyle = '->', connectionstyle='arc3,rad=0'))
                 #To be removed 
                if "doorframelines" in item and "Final Output With Bounding Box" in Title:
                    print ""
                    currentAxis = plt.gca()
                    currentAxis.add_patch(Rectangle((dfl_origin[0],dfl_origin[1]),dfl_hor_thr,dfl_ver_thr,edgecolor ='y',alpha=1,linewidth=3,fill=None,facecolor='grey') )
        
        #To be removed    
        if "Final Output With Bounding Box" in Title :#and not 'doorframe' in item:
            print "Debug Window(Ignore)"
            w = abs(right_bot_y - left_top_y)
            h = abs(right_bot_x - left_top_x)
            print right_bot_x
            print right_bot_y
            print left_top_x
            print left_top_y
            ...
            print h
            currentAxis = plt.gca()
            currentAxis.add_patch(Rectangle((left_top_x,left_top_y),h,w,edgecolor ='g',alpha=1,fill=None,facecolor='grey') )
        #Remove Till This
...
def relative_distance(result_df):
    ...
    for test_item in subfeature_dict:
        ...
        #Enable below comment to show plot for every feature
...
def doorframelines(result_df):
    ...
    print '\n*****************************Running: Between DoorFrameLines *****************************\n'  
...
def above_doorhandle(result_df):
    print '\n*****************************Running: Arrow & Window Above Handle *****************************\n'  
...
def physical_relative_distance(result_df):
    print '\n*****************************Running: Physical Relative Distance *****************************\n'
    ...
    for test_item in subfeature_dict:
        ...
        for item in subfeature_dict:
            if_valid = False
            if subfeature_dict_physical_dimension[test_item]:
                if subfeature_dict_physical_dimension[item] :
                    coordinates = subfeature_dict[item] 
                    dist = calc_rel_distance(test_coordinates, coordinates)
                    F =1000 #pixels
        ...
        toprint (subfeature_dict_rel_dist_cluster[test_item])    
        ...
        print test_item+':'+str(vote_count)+' Votes of '+str(length)+"Vote - False"
        result_df = update_result_df(result_df, test_item,'Vote',vote_count)
...
def main():
    ...
    for image_file_path in image_file_path_list:
        ...
        toprint(subfeature_height_width_list)
        ...
        print "*********************Opening Image:"+flightName+ ':' + imageName+"****************************\n"
        ...
        plot_points(image_file_path, subfeature_dict, subfeature_dict_final_grouping ,Title=('Final Output With Bounding Box:'+image_file_path))
