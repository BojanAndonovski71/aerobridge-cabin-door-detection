...
double thirdNum ,firstNumArrow , firstNum ;
int main() {
		cout<<"*****Between DoorFrame**********"<<endl;
		string config_path = "C:\\Config.ini";

		string rect_file_str = read_value("BetweenDoorFrame","rect_file",config_path);
		...
		char c2;
		  for ( int i = 0 ; i < 1; i ++)
			  ...
			  rect_file.get(c2);
			  	  		...
			  	  		rect_file.get(c2);
			  	  		rect_file.get(c2);
			  	  		rect_file >> fpvec2[i+2];
			  	  		rect_file.get(c2);
			  	  		rect_file >> fpvec2[i+3];
			  	  		rect_file.get(c2);
			  	  		rect_file.get(c2);

		          std::cout << fpvec2[i] <<" and "<< fpvec2[i+1] << " and "<< fpvec2[i+2] <<" and "<< fpvec2[i+3]<<std::endl;
		          double x1_rect=fpvec2[i];
		          double x2_rect=fpvec2[i+2];

		          string arrow_file_str = read_value("BetweenDoorFrame","arrow_file",config_path);
		          char arrow_file_char[100];
		          strcpy(arrow_file_char, arrow_file_str.c_str());
		ifstream arrow_file(TEXT(arrow_file_char),ios::app);
		          //ifstream arrow_file(,ios::app);
	...
	//Check if between frame lines
		if ( x2_rect < x_arrow < x1_rect ) {
			std::cout << "Between Frame Lines" << std::endl;
			cout<<"1"<<endl;
			 return 1;
			    }
			      else {
